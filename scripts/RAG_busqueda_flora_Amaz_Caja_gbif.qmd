...
install_github("musajajorge/mapsPERU")# solo hacer la primera vez, luego comentar con un "#" antes de la oración

# Packages names as a character vector
packages <- c("tidyverse", "rgbif", "sf","remotes",
...
En el siguiente "chunck" leemos la data retirada por Robert, seleccionamos los campos de interés (columnas), y removemos registros sin coordenadas.
```{r}
...
En el siguiente "chunck" hacemos un mapa de los registros en las regiones (la intensidad del color indica la cantidad de registros).
```{r mapa}

#plot data to get an overview
df <- map_REG# subset per regions
df <- dplyr::filter(df, REGION=="Cajamarca" |
                      REGION=="Amazonas")# filtering which regions
